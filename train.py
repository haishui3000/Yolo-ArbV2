...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), edges=edges, poly_out=poly_out).to(device)  # create
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors'), edges=edges, poly_out=poly_out).to(device)  # create
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in [-1, 0]:
                ...
                callbacks.run('on_train_batch_end', ni, model, imgs, targets, paths, plots, opt.sync_bn, edges)
            # end batch ------------------------------------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            if not noval and epoch%val_rate==0 or final_epoch:  # Calculate mAP
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/hrg.yaml', help='dataset.yaml path')
